$(function() {
    ...
    const btnLoadHapus = $("#loadButtonHapus");
    const btnSendHapus = $("#submitButtonHapus");
    ...
    $(formAdd).submit((e) => {
        if (formAdd.checkValidity()) {
            e.preventDefault();
            e.stopPropagation();
            btnSendTambah.toggleClass("d-none");
            btnLoadTambah.toggleClass("d-none");
    ...
    $(document).on('click', '.submitButtonHapus', function() {
        ...
        $.ajax({
            ...
            .fail((error) => {
                ...
                getPemasukkan(other);
                $(".notif-alert-universal").html(error);
                setTimeout(clearAlertUniversal, timeOut);
            })
    });
